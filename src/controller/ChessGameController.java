...
/**
 * This class manages the game logic for a chess game, including player
 * interactions, game state management, and communication with the chess engine.
 */
public class ChessGameController {
	...
	private ExecutorService executor; // Multi-thread that executes long-running tasks asynchronously to keep the UI
										// responsive
	...
	public ChessGameController(Board model, ChessGameView view) {
		...
		try {
			...
			this.stockfishEngine = new StockfishEngine(
					"./stockfish/stockfish/stockfish-windows-x86-64-sse41-popcnt.exe"); // Creates instance of chess
																						// engine from local directory
		} catch (IOException e) {
	...
	/**
	 * Configures the initial state of the UI, setting up listeners and displaying
	 * the initial status.
	 */
	...
	/**
	 * Processes button clicks on the chessboard, handling piece selection and
	 * movement.
	 *
	...
	/**
	 * Handles the logic for selecting a square. Ensures the selected square
	 * contains a piece of the current player's color.
	 *
	...
	private void handleSquareSelection(Square clickedSquare) {
		...
		if (piece != null && piece.getColor().equals(currentPlayer)) {
			...
			System.out.println("Selected piece: " + selectedSquare.getPiece() + " at " + selectedSquare.getX() + ", "
					+ selectedSquare.getY());
		} else {
	...
	/**
	 * Handles the process of attempting to move a piece. Validates and executes the
	 * move if valid.
	 *
	...
	private void handlePieceMovement(Square clickedSquare) {
		...
		if (piece != null) {
			System.out
					.println("Attempting to move piece: " + piece + " from " + selectedSquare + " to " + clickedSquare);
	...
	/**
	 * Validates the move and executes it if valid. Updates the move history,
	 * switches the player, and checks the game status.
	 *
	 * @param clickedSquare The square to which the piece is being moved.
	...
	/**
	 * Checks if the game is over or if the AI needs to make a move in single-player
	 * mode.
	 */
	...
	private void performAIMove() {
		...
		try {
			String bestMove = stockfishEngine.getBestMove(moveList); // Ask the chess engine for the best move and add
																		// it to the move list / update game state
	...
	/**
	 * Generates a string of all moves played in UCI format, suitable for use by the
	 * AI.
	 *
	...
	public void loadGame() throws ClassNotFoundException, IOException {
		...
		if (deserializeGameState()) {
			...
			// Update the view with the loaded model and current player
			view.updateBoard(model);
			view.updateStatusLabel("Current turn: " + currentPlayer);
	...
	private void applyMovesToModel() {
		...
		for (Move move : moveHistory) {
			...
			if (piece != null
					&& piece.movePiece(model.getSquare(move.getStartX(), move.getStartY()), endSquare, model)) {
